...
const ProductRow = ({ title, query, linkTo }) => {
  ...
  return (
    <div className="w-full bg-white sm:rounded-sm shadow-sm overflow-hidden mb-4 font-sans">
      {/* Header section */}
      <div className="flex justify-between items-center px-4 py-4 sm:px-6 sm:py-5 border-b border-gray-100">
        <h2 className="text-xl sm:text-2xl font-bold text-gray-800">{title}</h2>
        <Link 
          to={linkTo} 
          className="bg-[#2874f0] text-white px-4 py-1.5 sm:px-6 sm:py-2 rounded-sm text-xs sm:text-sm font-bold shadow hover:shadow-md transition-shadow"
        >
          VIEW ALL
        </Link>
      </div>
