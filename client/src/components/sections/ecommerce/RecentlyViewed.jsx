...
const RecentlyViewed = () => {
  ...
  useEffect(() => {
    // Fetch products for second row
    productsService.getProducts('?pageSize=6&pageNumber=2')
      .then(res => {
  ...
  return (
    <div className="w-full bg-white sm:rounded-sm shadow-sm overflow-hidden mb-8">
      {/* Header section */}
      <div className="flex justify-between items-center px-4 py-4 sm:px-6 sm:py-5 border-b border-gray-100">
        <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Top Deals on Fashion</h2>
        <Link 
          to="/shop?offers=true" 
        ...
        {products.map((product) => (
          ...
          >
            ...
            <p className="text-[#388e3c] text-base font-bold text-center">
              {product.discountPercentage > 0 ? `Min. ${product.discountPercentage}% Off` : `₹${Math.round(product.discountPrice || product.price).toLocaleString('en-IN')}`}
            </p>
          </Link>
        ))}
      </div>
